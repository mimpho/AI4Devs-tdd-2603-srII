...
5. Se pidio crear archivos de tests separados por capas:
...
6. Se pidio crear este archivo de prompts en la carpeta `prompts/prompts-MTT.md`.
...
3. **Frontend (7 tests):**
   - Mockeamos `react-datepicker` para controlar fechas deterministicamente
...
4. **Backend (19 tests):**
   - Se habilito `ts-jest` con config local en `backend/jest.config.js`
...
2. Se mantienen tambien los tests unitarios por componente/capa porque son mas utiles para mantenimiento, depuracion y evolucion del codigo.
3. La coexistencia es intencional y documentada: no implica reemplazo de una estrategia por la otra.
4. En ejecucion de Jest se detectan ambas suites; esto es esperado mientras se mantenga este criterio.
